...
# class OllamaClient:
#     def __init__(self, model=" qwen3-vl:4b", url="http://localhost:11434/api/generate"):
#         self.model = model
#         self.url = url
...
class DXFGenerator:
    ...
    def generate_dxf(self, parsed_data, file_path):
        try:
            ...
            # Setup units (important for AutoCAD)
...
class DiagramCanvas(QWidget):
    ...
    def generate_from_prompt(self, prompt_text):
        """Generate diagram from text prompt using Ollama (LLM) first, regex fallback second"""
        try:
            ...
            if not prompt_text:
                QMessageBox.warning(
                    self,
                    "Empty Prompt",
                    "Please enter a diagram description."
                )
                return False
            ...
            # ===============================
            # STEP 1: Try Ollama (LLM parsing)
            # ===============================
            try:
                ollama = OllamaClient(model="qwen2.5:1.5b")
                parsed_data = ollama.prompt_to_structured_data(prompt_text)

                # Basic validation to avoid bad LLM output
                if (
                    not isinstance(parsed_data, dict)
                    or "components" not in parsed_data
                    or not isinstance(parsed_data["components"], list)
                ):
                    raise ValueError("Invalid structured data from LLM")

                print("DEBUG: Parsed via Ollama")

            except Exception as llm_error:
                # ===============================
                # STEP 2: Regex fallback (existing logic)
                # ===============================
                print(f"LLM parsing failed, falling back to regex parser: {llm_error}")
            ...
            print(f"DEBUG: Mermaid code length: {len(mermaid_code)}")

            # ===============================
            # STEP 4: Generate HTML (editable)
            # ===============================
            html_content = self.generator.generate_display_html(
                mermaid_code,
                parsed_data,
                enable_editing=True
            )
            ...
            if self.parent_window and hasattr(self.parent_window, "status"):
                ...
                self.parent_window.status.showMessage(
                    f"Diagram generated via LLM ({lang_name}), "
                    f"{component_count} components. Double-click to edit.",
                    4000
                )
