...
def route_diagram_wire(src_cid: str, dst_cid: str, wtype: str, qt_src: tuple, qt_dst: tuple) -> list:
    """Generate canonical Manhattan wire points matching DXF routing pipeline."""
    try:
        from ECD.electrical.wire_router import route_wire_bend
    except ImportError:
        from electrical.wire_router import route_wire_bend
...
# EditableTextItem imported from ECD.canvas.text_item
...
class DiagramGroupItem(QGraphicsItemGroup):
    ...
    def mouseMoveEvent(self, event):
        if self.is_resizing:
            ...
            event.accept()
        elif getattr(self, "_press_scene_pos", None) and (event.buttons() & Qt.MouseButton.LeftButton):
            delta_vec = event.scenePos() - self._press_scene_pos
            if not getattr(self, "_drag_threshold_exceeded", False):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.is_resizing:
            ...
            if getattr(self, "_drag_threshold_exceeded", False) and getattr(self, "_initial_positions", None):
                ...
                for item, start_pos in self._initial_positions.items():
                    ...
                    if cid is not None:
                        ...
                        old_item_p = start_pos
                        old_item_s = item.scale()
                        if hasattr(self.parent_view, "store_layout_override"):
                            self.parent_view.store_layout_override(item.name, item.pos(), item.scale(), old_item_p, old_item_s)
...
# SymbolItem imported from ECD.canvas.symbol_item
...
class SvgPreviewWidget(QGraphicsView):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in (Qt.Key.Key_Left, Qt.Key.Key_Right, Qt.Key.Key_Up, Qt.Key.Key_Down):
            ...
            if not selected_symbols and not selected_groups:
                super().keyPressEvent(event)
                return
...
class DiagramCanvas(QWidget):
    ...
    @property
    def scene(self):
        return self.svg_widget.scene if hasattr(self, "svg_widget") else None

    def toggle_symbol_lock(self):
        if hasattr(self, "svg_widget") and self.svg_widget.scene:
            selected = [item for item in self.svg_widget.scene.selectedItems() if hasattr(item, "toggle_lock")]
            for item in selected:
                item.toggle_lock()
